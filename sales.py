...
layout=[[sg.Text("Analysis")],
    [sg.Text('buying amount'),
     sg.InputText(key="_number1_",size=(20,10)),
     sg.Text('sales amount'), sg.InputText(key="_number2_",size=(20,10)),
     sg.Drop(values=(mnt),key="_listbox_", auto_size_text=True,default_value=mnt[0]),
     sg.Button("Save information",key="_button_"),
     sg.Button("comparison graphic",key="_graphic_")]]
...
while True:
 ...
 if event=="_button_":
      if values["_number1_"].isalpha() or values["_number2_"].isalpha():
          ...
          gain = round((float(values["_number2_"]) - float(values["_number1_"])) * 100) / (float(values["_number1_"]))
          for k in str(gain):
              gainValues += k
